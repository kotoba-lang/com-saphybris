(ns saphybris.main
  "Kotodama WASM entrypoint for the Saphybris clean-room actor (L4) — Clojure port.
  ...
  facts (`saphybris.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "saphybris")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"saphybris.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "saphybris-compat" :tier tier :entities entities} 200])
...
(defn start! [] :saphybris-compat/ready)
